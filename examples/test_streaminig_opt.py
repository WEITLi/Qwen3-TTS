...
def main():
    ...
    ref_audio_path = "/data/Projects/Step-Audio-Edit-Benchmark/prompt_audios/fuxi/fuxi_02.wav"
    ref_text = (
        "在我眼里，天下没有麻烦的事，不过弹弹手指就能解决，若说有麻烦，那便是你这个人。"
    )
    ...
    # Test text
    test_text = "小兔子蹦蹦跳跳地来到魔法森林，它竖起长长的耳朵，用软软的鼻子碰了碰发光的蘑菇：“蘑菇先生，您知道哪里能找到彩虹糖吗？"
    ...
    sf.write("output/streaming/output_standard.wav", wavs[0], sr)
    results.append({
        "label": "standard",
    ...
    result = run_streaming_test(
        model, test_text, "Chinese", voice_clone_prompt,
        emit_every_frames=EMIT_EVERY,
        decode_window_frames=DECODE_WINDOW,
        label="streaming_baseline",
    )
    results.append(result)
    sf.write("output/streaming/output_streaming_baseline-ref-fixed.wav", result["audio"], result["sample_rate"])
    ...
    warmup_result = run_streaming_test(
        model, "小兔子蹦蹦跳跳地来到魔法森林", "Chinese", voice_clone_prompt,
        emit_every_frames=EMIT_EVERY,
        decode_window_frames=DECODE_WINDOW,
        label="warmup",
    )
    ...
    results.append(result)
    sf.write("output/streaming/output_streaming_optimized-ref-fixed.wav", result["audio"], result["sample_rate"])
